...
int cartoTrajOutputCheck(std::string path) {
...
bool RunOfflineNode(const MapBuilderFactory& map_builder_factory, std::string& bag_name, std::string& pbstream_name) {
  CHECK(!FLAGS_configuration_directory.empty())
      << "-configuration_directory is missing.";
  CHECK(!FLAGS_configuration_basenames.empty())
      << "-configuration_basenames is missing.";
  CHECK(!FLAGS_save_state_filename.empty())
      << "-save_state_filename is missing.";
  ...
  bag_name = bag_filenames.front();
  pbstream_name = FLAGS_save_state_filename;

  size_t slash_pos = bag_name.find_last_of("/");
